...
#' @export
function (grid, n, steps, m, nu) 
{
    df <- expand.grid(x = 1:n, y = 1:n)
    df$species <- as.vector(t(grid))
    df$species_f <- factor(df$species)
    ggplot(df, aes(x = x, y = y, fill = species_f)) + geom_raster() + 
        coord_equal() + labs(title = "Spatial Neutral Model (grid community map)", 
        subtitle = paste0("n = ", n, ", steps = ", steps, "\n", 
            "m = ", m, ", ν = ", nu), x = "X", y = "Y", fill = "Species") + 
